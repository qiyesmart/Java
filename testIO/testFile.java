...
public class testFile {
    ...
    private static String computerPrefix(int level){
    ...
    private static  void printAllFiles(File file,int level){
        if(file.isDirectory()){
            File[] files=file.listFiles();
            if(files!=null){
                for(File f:files){
                    System.out.println(computerPrefix(level)+f.getName());
                    printAllFiles(f,level+1);
                }
            }
        }
    }
